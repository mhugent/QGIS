...
bool WebDataModel::exchangeLayer( const QString& layerId, QgsMapLayer* newLayer )
{
  ...
  QgsLayerTreeLayer* mlTreeLayer = mIface->layerTreeView()->layerTreeModel()->rootGroup()->findLayer( newLayer->id() );
  QgsLayerTreeLayer* afterTreeLayer = mIface->layerTreeView()->layerTreeModel()->rootGroup()->findLayer( oldLayer->id() );
  if ( !mlTreeLayer || !afterTreeLayer )
    ...
    afterTreeGroup->insertLayer( afterIndex + 1, const_cast<QgsMapLayer*>( newLayer ) ); //that function should really accept a const layer
